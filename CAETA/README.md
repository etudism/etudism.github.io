...
### ルート
- `index.html` : DOM 本体
- `styles/01-main.css` : スタイル全体
- `scripts/01-data.js` : 巨大な `DATA`
...
### 依存
- `scripts/lib/01-deps.js` : 本番用。`three` / `OrbitControls` を CDN から読み込む

### core
- `scripts/core/01-errors.js`
  - グローバルエラー表示の登録
- `scripts/core/02-dom.js`
  - DOM 参照の一括取得

### utils
- `scripts/utils/01-id.js`
  - `normalizeBoothId`
  - `escapeHtml`
  - `normalizeSearchText`
- `scripts/utils/02-groups.js`
  - 選択 ID の正規化
  - group key 解決
  - group 単位の tier 集約

### features
- `scripts/features/01-search-utils.js`
  - 検索インデックス文字列
  - 検索スコア計算
- `scripts/features/02-search-ui.js`
  - 検索結果 UI の描画
  - 検索更新 / クリア
- `scripts/features/03-meta-utils.js`
  - 情報パネルのメタ文字列生成
- `scripts/features/04-selection-core.js`
  - 選択 tier 判定
  - 選択サマリー更新
  - 選択解除補助
- `scripts/features/05-category-utils.js`
  - カテゴリ正規化
  - カテゴリ色計算
  - 凡例エントリ生成
- `scripts/features/06-category-layer.js`
  - カテゴリ凡例 UI
  - カテゴリレイヤー表示切替
  - カテゴリ用 material 生成
- `scripts/features/07-drawer.js`
  - ドロワー一覧描画
  - ドラッグ & ドロップ並び替え
  - ドロワー開閉
- `scripts/features/08-url-state.js`
  - URL 状態の純粋関数
- `scripts/features/09-persistence.js`
  - localStorage 保存 / 復元
  - URL 共有状態反映
  - camera 状態復元
- `scripts/features/10-action-menu.js`
...
### visuals
- `scripts/visuals/01-textures-and-labels.js`
  - テキストテクスチャ
  - スプライト
  - 床文字
  - ポータル三角形
  - コンクリートテクスチャ
- `scripts/visuals/02-hall-architecture.js`
  - 会場外装・柱・梁・シャッター・番号塔など
- `scripts/visuals/03-layout-and-overlay.js`
  - ホール座標変換
  - ホール床面生成
  - 特設矩形生成
  - 外枠マーカー
...
この環境では外部 DNS 制約のため、本番用 `01-deps.js` が読む CDN (`esm.sh`) には到達できません。
そのため、**本番コードはそのままに**、検証時のみ `tests/01-deps-stub.js` を差し込む方式で Playwright 実行確認を行っています。
...
```bash
python3 tests/02-verify-runtime.py
```
