export function setupLayoutAndOverlay({
  THREE,
  DATA,
  document,
  renderer,
  ...
  function layoutX(page) {
    return HALL_OFFSETS[getLayoutMode()][page];
  }
  ...
  function getAreaLabelOffset(band) {
    ...
    if ((band.page === 1 && eastShiftPage1.has(label)) || (band.page === 2 && eastShiftPage2.has(label))) dx += 10;
    if (band.page === 1 && label === 'A') dy -= 10;
    if (band.page === 2 && label === 'あ') dy -= 10;
    if (band.page === 1 && label === 'X') dx -= 14;
    if (band.page === 2 && label === 'ね') dx -= 14;
    return { dx, dy };
  }
  ...
  function rebuildOverlayData(state) {
    state.overlayBoothLabels = [];
    state.overlayAreaLabels = [];
  }
  ...
  function drawTextLabel(labelCtx, text, x, y, fontSizePx, fillStyle, strokeStyle, lineWidth) {
  ...
  function renderOverlayLabels(labelCtx) {
