...
のように、**番号付きプレフィックスが整理ルールとして使われています**。
...
そのため、安易に番号だけ変えたり、規則を崩したりしない方が安全です。

## 3-2. `00-app.js` を何でも屋に戻さない
現在は機能ごとに分かれています。  
小さな修正でも、なるべく該当 feature / visuals module に寄せてください。

`00-app.js` は**統合と接着**に寄せ、個別ロジックを増やしすぎない方が保守しやすいです。

## 3-3. `index.html` と `20-dom.js` を必ずそろえる
HTML 側で要素を追加・削除したのに DOM 参照側を直さないと、すぐ不整合が起きます。

## 3-4. `main` に直接コミットしない
GitHub Pages 用リポジトリは、`main` を壊すと公開物まで壊れやすいです。  
**作業は必ず branch で行います。**
